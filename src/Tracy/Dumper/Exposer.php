...
final class Exposer
{
	...
	public static function exposeArrayObject(\ArrayObject $obj, Structure $struct, Describer $describer): void
	{
		$flags = $obj->getFlags();
		if (!($flags & \ArrayObject::STD_PROP_LIST)) {
			$obj->setFlags(\ArrayObject::STD_PROP_LIST);
			self::exposeObject($obj, $struct, $describer);
			$obj->setFlags($flags);
		}
		$describer->addProperty($struct, 'storage', $obj->getArrayCopy(), \ArrayObject::class);
	}
