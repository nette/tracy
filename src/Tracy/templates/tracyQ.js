...
(function(){
	...
	Query.prototype._trav = function(el, selector, fce) {
		while (el && selector && !(el.nodeType === 1 && (el.matches || el.matchesSelector || el.msMatchesSelector || el.mozMatchesSelector || el.webkitMatchesSelector || el.oMatchesSelector).call(el, selector))) {
			el = el[fce];
		}
		return new Query(el || []);
	};
