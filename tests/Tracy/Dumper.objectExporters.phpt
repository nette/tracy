...
$exporters = [
	'stdClass' => function ($var, Value $value, Dumper\Describer $describer) {
		$describer->addPropertyTo($value, 'x', $var->a + 2, Value::PropPublic);
		...
		$describer->addPropertyTo($inner, 'a', 'b', Value::PropPublic);
...
$exporters = [
	'stdClass' => function ($var, Value $value, Dumper\Describer $describer) {
		$describer->addPropertyTo($value, 'x', 'y', Value::PropPublic);
