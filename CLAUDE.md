...
- Agent detection: `Helpers::isAgent()` checks `$_COOKIE['tracy-webdriver']` — set by `bar.js` when `navigator.webdriver` is `true` (Chrome MCP, Playwright, Puppeteer, Selenium)

### Core Components

**BlueScreen** (src/Tracy/BlueScreen/)
- Error/exception page rendering with stack traces
- Multiple output formats: HTML, CLI, markdown (for AI agents via console)
- `renderAgent()` returns markdown string, output via `console.error()` in page `<script>`
- `renderToAjax()` also sends agent markdown via `console.error` for AJAX exceptions
- `renderToFile()` creates both `.html` and `.md` (markdown) log files in production
- `getDumper()` returns HTML closure, `getAgentDumper()` returns text closure (depth 3, scrubber, keysToHide)
...
- `renderAgent()` returns markdown string, output via `console.log()` in injected `<script>`
- Agent output works for normal HTML, AJAX (via `addSetup`), and redirect requests (stored in redirect queue)
- `dump()` also outputs `console.log()` for agents; `bdump()` text generated only when agent detected
...
- File-based error logging: `{level}.log` text lines + `{level}--{date}--{hash}.html` + `.md` companion
- Email notifications via `ILogger` interface
- Severity filtering
- Hash-based deduplication (same exception = same file, no overwrite)
...
- Production error page (`error.500.phtml`) includes `navigator.webdriver` check → `console.error()` with generic message (no sensitive data)
